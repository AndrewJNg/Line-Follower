...
void junction(int speedL_junction, int speedR_junction, float kp, float kd, char type, char action, int action_delay, int turn_speed, int turn_time){
  switch (type){
   case 'L':
             do{
               pid_line(speedL_junction, speedR_junction, kp,0, kd);
             }
             while(get_state[0] == 0 || get_state[3] == 0);  // left junction 111xx
   ...
   case 'M':
             ...
             while(get_state[0] == 0 || get_state[3] == 0 || get_state[6] == 0);  // middle junction 1xxx1
             forward(speedL_junction, speedR_junction);
             delay(action_delay);
             choose_action(action, turn_speed, turn_time);         
             break;
   case 'R':
             ...
             while(get_state[3] == 0 || get_state[6] == 0);  // right junction xx111 
             forward(speedL_junction, speedR_junction);
             delay(action_delay);
             choose_action(action, turn_speed, turn_time);         
             break;
