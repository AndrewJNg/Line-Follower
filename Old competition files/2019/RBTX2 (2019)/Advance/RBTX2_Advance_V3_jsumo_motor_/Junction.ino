...
void Junction(int speedL_junction, int speedR_junction, float kp, float ki, float kd, char type, char action)
{ switch (type) {
    case 'L':
      do {
        PID(speedL_junction, speedR_junction, kp, ki, kd);
      }
      while (get_state[0] == 0 ); // left junction 111xx
      choose_action(action, speedL_junction,  speedR_junction, kp, ki,  kd);
      break;
    case 'M':
      do {
        PID(speedL_junction, speedR_junction, kp, ki, kd);
