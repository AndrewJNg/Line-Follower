...
{ switch (type) {
    ...
    case 'M':
      ...
      while (get_state[0] == 0 ||  get_state[6] == 0); // middle junction 1xxx1
      choose_action(action, speedL_junction,  speedR_junction, kp,  kd);
      break;
    case 'R':
      do {
      ...
      while ( get_state[6] == 0); // right junction xx111
      choose_action(action, speedL_junction,  speedR_junction, kp,  kd);
      break;
    ...
    case 'U':
      do {
        PID(speedL_junction, speedR_junction, kp, kd);
      } while (((distanceSensor.measureDistanceCm() > 20 ) || (distanceSensor.measureDistanceCm() < 0)));
      choose_action(action, speedL_junction,  speedR_junction, kp,  kd);
      break;
