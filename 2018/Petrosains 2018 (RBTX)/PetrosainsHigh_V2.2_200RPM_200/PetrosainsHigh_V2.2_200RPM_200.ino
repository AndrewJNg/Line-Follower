...
int SetPath[] = {LW
  //  R,L,LW,B,LW,R,S,R,R,LW,R,L
  //R, L, RW, L, L, RW, L, RW, B, RW, R, L, S
 // R, L, B, R, L, B, RW, LW, B, RW, LW
};
...
void LeftWall() {
  brake();
  timer = millis();
  while ((millis() - timer) < (1200 + moveTime2)) {
    wallFollowLeft();
  }
  /*timer = millis();
    while ((analogRead(IR0) > 500) && ((millis() - timer) < moveTime2)) {
    wallFollowLeft();
    }*/
  brake();
}
...
void Left() {
  ...
  timer = millis();
  while ((analogRead(IR0) > 500) && ((millis() - timer) < moveTime)) {
    wallFollowRight();
  }
  brake();
}
